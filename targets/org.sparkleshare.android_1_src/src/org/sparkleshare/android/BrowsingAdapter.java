...
public class BrowsingAdapter extends BaseAdapter {
  ...
  public View getView(int position, View convertView, ViewGroup parent) {
    ...
    if (convertView == null) {
      view =
        LayoutInflater.from(context).inflate(R.layout.list_item_entry, null);
    ...
    File file = new File(
      ExternalDirectory.getExternalRootDirectory() + "/" + item.getTitle());
    if (file.exists()) {
    ...
    if (item.getMimetype() != null) {
      viewHolder.icon.setImageResource(
        MimetypeChecker.getResIdforMimetype(item.getMimetype()));
    }
    return view;
  }
