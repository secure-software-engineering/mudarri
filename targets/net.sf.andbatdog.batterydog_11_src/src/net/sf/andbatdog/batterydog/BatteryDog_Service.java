...
public class BatteryDog_Service extends Service {
  ...
  private final static String[] batteryExtraKeys =
    {"level", "scale", "voltage", "temperature", "plugged", "status", "health",
      "present", "technology", "icon-small"};
  ...
  public void onCreate() {
    ...
    if (!mThreadRunning) {
      ...
      Toast.makeText(this, "BatteryDog Service started", Toast.LENGTH_SHORT)
        .show();
    }
  }
  ...
  public void onDestroy() {
    ...
    Toast.makeText(this, "BatteryDog Service stopped", Toast.LENGTH_SHORT)
      .show();
  }
    ...
    try {
      ...
      if (out == null) {
        ...
        if (!mBatteryLogFile.exists())
          throw new Exception(
            "creation of file '" + mBatteryLogFile.toString() + "' failed");
        if (!mBatteryLogFile.canWrite())
          throw new Exception(
            "file '" + mBatteryLogFile.toString() + "' is not writable");
        out = new FileWriter(mBatteryLogFile, true);
        if (!fileExists) {
  ...
  private String createBatteryInfoLine(Intent batteryIntent) {
    StringBuffer result = new StringBuffer();
    result.append(Integer.toString(mCount)).append(";")
